  ...
  handler: async (input, context) => {
    ...
    result.forEach(element => {
      if (element.Identifier == "List") {
        let sql = 'INSERT INTO MailChimpLists (ListName, Description, DefaultName, DefaultEmail, DefaultSubject, DateCreated,Language, Url) VALUES (?,?,?,?,?,?,?,?)';
        let values = [element.List_Name, element.Permission_reminder, element.Campaign_defaults.from_name, element.Campaign_defaults.from_email, element.Campaign_defaults.subject, element.Date_created, element.Url];
